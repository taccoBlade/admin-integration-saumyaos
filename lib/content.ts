...
function mapProject(row: SupabaseProjectRow): Project {
  ...
  return {
    ...
    identity: source.identity as Project["identity"] || {
      accent: "#6366f1",
      background: "#08090b",
      surface: "#111216",
      typography: "sans",
...
export const getProject = (id: string) => unstable_cache(
...
export const getResearchEntries = unstable_cache(
  async (): Promise<ResearchEntryData[]> => {
...
export const getPhotoSpreads = unstable_cache(
  async (): Promise<SpreadData[]> => {
    try {
      const supabase = createSupabasePublicClient();
