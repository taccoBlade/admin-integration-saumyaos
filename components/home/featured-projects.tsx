...
export function FeaturedProjects({ projects, showConsoles = true }: { projects: Project[], showConsoles?: boolean }) {
  return (
    <section id="projects" className="relative w-full max-w-7xl mx-auto px-5 py-24 sm:px-8 lg:px-12">
      
      {/* Featured Projects Segment */}
      <ScrollReveal variant="fadeUp" className={showConsoles ? "mb-12" : "mb-8"}>
      ...
      {showConsoles && (
        <>
          ...
          <ScrollReveal variant="fadeUp" className="mb-10">
            <h3 className="text-2xl sm:text-3xl font-semibold text-white tracking-tight">Project Dashboards</h3>
            <p className="text-slate-400 mt-3 text-sm max-w-2xl">Explore live data dashboards and interactive models for selected projects.</p>
          </ScrollReveal>

          <ScrollRevealStagger stagger={0.1} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {projects.map((project) => (
              <RevealItem key={`vault-${project.id}`}>
                <Link href={`/projects?project=${project.id}`} className="block group h-full">
                  <motion.div
                    variants={hoverLift}
                    initial="rest"
                    whileHover="hover"
