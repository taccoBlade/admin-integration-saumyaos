...
interface HeroHistoryViewProps {
  versions: HeroVersion[];
  loading: boolean;
  isPending: boolean;
  onBack: () => void;
  onRollback: (ver: HeroVersion) => void;
}

export default function HeroHistoryView({
  versions,
  loading,
  isPending,
  onBack,
  onRollback,
}: HeroHistoryViewProps) {
  return (
    <div className="space-y-6 max-w-xl font-mono text-xs select-none">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-white">Version Snapshot Registry</h1>
          <p className="text-[10px] text-[var(--muted)]">HERO REVISION HISTORY</p>
        </div>
      </div>

      <div className="border border-white/5 bg-[#0c0d12]/50 p-6 rounded-2xl">
        {loading ? (
          ...
          <ul className="space-y-4">
            {versions.map((ver) => (
              <li key={ver.id} className="flex justify-between items-center py-3 border-b border-white/5 last:border-0 gap-4">
